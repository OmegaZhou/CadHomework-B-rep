...
bool ZLib::ImageBuffer::saveImage(std::string path) const
{
	std::string str;
...
bool ZLib::ImageBuffer::saveImage(std::wstring path) const
{
	std::string str;
	std::ostringstream string_os(std::ostringstream::binary | std::ostringstream::out);
	std::ofstream os;
...
{
	if (width * height < w * h||frame_buffer==nullptr) {
		freeBuffer();
...
ZLib::ImageBuffer::ImageBuffer(const ImageBuffer& buffer)
{
	*this = buffer;
}
